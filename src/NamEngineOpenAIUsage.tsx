...
function SessionCostTable({ rows }: { rows: SessionUsageMetric[] }) {
  ...
  return <article className="telemetry-panel telemetry-panel-wide"><div className="panel-heading"><div><p className="eyebrow">Sessions</p><h2>Estimated cost by session</h2></div><span>{rows.length ? `${formatNumber(rows.length)} sessions` : 'No session rows yet'}</span></div>{visibleRows.length ? <><div className="session-cost-cards">{visibleRows.map((row) => <div className="session-cost-card" key={row.session_id}><div><strong>{formatCurrency(row.estimated_spend_usd, 4)}</strong><span>{formatNumber(row.request_count)} requests · {formatNumber(row.total_tokens)} tokens</span></div><p title={row.session_id}>{shortSessionId(row.session_id)}</p><dl><div><dt>Type</dt><dd>{shortRequestTypes(row.request_types)}</dd></div><div><dt>Vertical</dt><dd>{row.vertical}</dd></div><div><dt>Model</dt><dd>{row.model}</dd></div><div><dt>Date</dt><dd>{formatDate(row.date)}</dd></div></dl></div>)}</div><div className="usage-table-wrap session-cost-table-wrap"><table className="usage-table session-cost-table"><thead><tr><th>Session</th><th>Type</th><th>Date</th><th>Vertical</th><th>Model</th><th>Requests</th><th>Tokens</th><th>Names</th><th>Avg. latency</th><th>Estimated cost</th></tr></thead><tbody>{visibleRows.map((row) => <tr key={row.session_id}><td><span className="session-id-cell" title={row.session_id}>{shortSessionId(row.session_id)}</span></td><td><span className="request-type-cell" title={row.request_types.join(', ')}>{shortRequestTypes(row.request_types)}</span></td><td>{formatDate(row.date)}</td><td>{row.vertical}</td><td>{row.model}</td><td>{formatNumber(row.request_count)}</td><td>{formatNumber(row.total_tokens)}</td><td>{formatNumber(row.generated_name_count)}</td><td>{formatLatency(row.average_latency_ms)}</td><td>{formatCurrency(row.estimated_spend_usd, 4)}</td></tr>)}</tbody></table></div></> : <p className="empty-panel-copy">Deploy the latest NamEngine telemetry API so Mission Control can receive per-session rows.</p>}</article>
}
